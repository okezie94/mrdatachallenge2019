...
mvmr2 <- function(r_input, gencov, weights){
  ...
  Q_valid_vct <- (1/sigma2A)*(r_input[,2]-temp.sub2)^2
  Q_valid<- sum (Q_valid_vct)
  
  #Calculates p_value for instrument validity
  Q_chiValid<-pchisq(Q_valid,length(r_input[,2])-exp.number-1,lower.tail = FALSE)
  
  # p-values for Q-statistic contributions
  Qj_valid_vct_Chi <- numeric(length(Q_valid_vct))
  for(i in 1:length(Q_valid_vct)){
    Qj_valid_vct_Chi[i] <- pchisq(Q_valid_vct[i], 1, lower.tail = FALSE)
  }
  ...
  multi_return <- function() {
	...
	"Q_valid_vct" = Q_valid_vct, "Q_valid_vct_Chi" = Qj_valid_vct_Chi)
